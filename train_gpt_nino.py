...
def mm_op(x: Tensor, w: Tensor, x_s: float, w_s: float, grad_s: float) -> tuple[Tensor, Tensor, Tensor]:
    ...
    def impl(x: Tensor, w: Tensor):
        ...
        # replace _scaled_mm with mm and float8 with bfloat16 for A100 and older cuda
        x_f8 = x.div(x_s).to(torch.bfloat16)
        w_f8 = w.div(w_s).to(torch.bfloat16)
        out = torch.mm(x_f8, w_f8.T) * (x.new_tensor(x_s, dtype=torch.float32) * x.new_tensor(w_s, dtype=torch.float32))
        return out.to(torch.bfloat16), x_f8, w_f8
...
def mm_backward_op(g: Tensor, x_f8: Tensor, w_f8: Tensor, x_s: float, w_s: float, grad_s: float) -> tuple[Tensor, Tensor]:
    ...
    def impl(grad: Tensor, x_f8: Tensor, w_f8: Tensor):
        ...
        grad_x = torch.mm(grad_f8, w_f8.T.contiguous().T) * (grad_inv_s * w_inv_s)
        grad_w = torch.mm(x_f8.T.contiguous(), grad_f8.T.contiguous().T) * (x_inv_s * grad_inv_s)
        return grad_x.to(torch.bfloat16), grad_w.to(torch.float32).T
...
class Hyperparameters:
    ...
    train_seq_len = 2*48*1024  # FlexAttention sequence length (multiply by 2 to maximize GPU utilization)
    val_seq_len = 4*64*1024  # FlexAttention sequence length for validation
    # optimization
    num_iterations = 1770*4  # number of iterations to run (increase by 4 to make it similar to 1770 iterations on 8 GPUs)
...
assert world_size == 1 # this code is designed for 1xA100
